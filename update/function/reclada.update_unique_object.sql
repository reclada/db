...
RETURNS bool AS $$
BEGIN
    IF array_length(guid_list,1) = 0 THEN
        recalc_all := true;
    END IF;
    WITH obj_unique_fields AS MATERIALIZED(
        SELECT  vao.id, b.f_path, b.f_type FROM reclada.v_active_object vao 
        JOIN reclada_object.explode_jsonb(attrs,'attributes' ) b ON 1=1
    ...
    objects_with_fields AS (
        SELECT array_agg(f.id) AS id_field, ouf.id
        FROM obj_unique_fields ouf
