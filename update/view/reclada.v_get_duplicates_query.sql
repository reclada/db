...
AS
SELECT
'SELECT ''SELECT vao.obj_id, '''''' || dup_behavior || ''''''::dp_bhvr,'' || is_cascade || '','' || COALESCE (copy_field,'''''''''''') ||'' FROM reclada.v_active_object vao WHERE '' ||  string_agg(predicate, '' OR '') @#@#@exclude_uuid@#@#@
