...
BEGIN
    ...
    class := data->>'{class}';
    IF(class IS NULL) THEN
        RAISE EXCEPTION 'reclada object class not specified1';
    END IF;
...
CREATE VIEW reclada.v_object_display AS
 SELECT obj.id,
    obj.guid,
    ((obj.attributes ->> 'сlassGUID'::text))::uuid AS class_guid,
    (obj.attributes ->> 'caption'::text) AS caption,
...
SELECT pg_catalog.setval('reclada.object_id_seq', 66, true);
...
SELECT pg_catalog.setval('reclada.transaction_id', 66, true);
