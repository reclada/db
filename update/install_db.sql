...
--
-- Name: update_unique_object(uuid[]); Type: FUNCTION; Schema: reclada; Owner: -
--

CREATE FUNCTION reclada.update_unique_object(guid_list uuid[]) RETURNS boolean
    LANGUAGE plpgsql
    AS $$
DECLARE
    ...
    IF gui THEN
        ...
        if ver = '2' then
            ...
            if class_uuid is not null then
                ...
                _exec_text := _pre_query ||',
                dd as (
                    select distinct f.path||''#@#@#separator#@#@#''||f.json_type v
                        FROM '|| _from ||'
...
SELECT pg_catalog.setval('reclada.field_id_seq', 1, false);
...
SELECT pg_catalog.setval('reclada.unique_object_id_seq', 1, false);
...
SELECT pg_catalog.setval('reclada.unique_object_reclada_object_id_seq', 1, false);
