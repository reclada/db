...
--
-- Name: add_cr_dup_mark(uuid, bigint, reclada.dp_bhvr); Type: FUNCTION; Schema: reclada_object; Owner: -
--

CREATE FUNCTION reclada_object.add_cr_dup_mark(_parent_guid uuid, _transaction_id bigint, _dup_behavior reclada.dp_bhvr) RETURNS void
    LANGUAGE plpgsql
    AS $$
BEGIN
...
--
-- Name: built_nested_jsonb(text[], jsonb); Type: FUNCTION; Schema: reclada_object; Owner: -
--

CREATE FUNCTION reclada_object.built_nested_jsonb(_path text[], _value jsonb) RETURNS jsonb
    LANGUAGE plpgsql IMMUTABLE
    AS $$
DECLARE
    n        integer;
...
CREATE MATERIALIZED VIEW reclada.v_class_lite AS
 WITH RECURSIVE objects_schemas AS (
        ...
        UNION ALL
         ...
         SELECT reclada_object.built_nested_jsonb(t.path_head[(array_position(t.path_head, 'properties'::text) + 1):], (t.path_tail -> 'default'::text)) AS default_jsonb,
            t.obj_id
           FROM paths_to_default t
          WHERE ((t.path_tail -> 'default'::text) IS NOT NULL)
        ), default_field AS (
         SELECT (format('{"attributes":%s}'::text, json_object_agg(def_1.default_key, def_1.default_value)))::jsonb AS default_value,
            def_1.obj_id
           FROM ( SELECT tmp.obj_id,
                    d.key AS default_key,
                    d.value AS default_value
                   FROM tmp,
                    LATERAL jsonb_each(tmp.default_jsonb) d(key, value)) def_1
          GROUP BY def_1.obj_id
        )
 SELECT obj.id,
    obj.obj_id,
...
--
-- Name: v_filter_avaliable_operator; Type: VIEW; Schema: reclada; Owner: -
--

CREATE VIEW reclada.v_filter_avaliable_operator AS
 ...
 SELECT b.for_class,
   ...
   FROM ( SELECT a.for_class,
           ...
           FROM ( SELECT vc.for_class,
                    (vc.attrs ->> 'dupBehavior'::text) AS dup_behavior,
                    (vc.attrs ->> 'isCascade'::text) AS is_cascade,
                    jsonb_array_elements((vc.attrs -> 'dupChecking'::text)) AS dc,
                    vc.obj_id
                   FROM reclada.v_class vc
                  WHERE ((vc.attrs -> 'dupChecking'::text) IS NOT NULL)) a) b
  WITH NO DATA;
...
CREATE VIEW reclada.v_parent_field AS
 SELECT v_class.for_class,
    ...
    (v_class.attrs ->> 'parentField'::text) AS parent_field
   FROM reclada.v_class
  WHERE ((v_class.attrs ->> 'parentField'::text) IS NOT NULL);
 ...
 SELECT obj.obj_id AS guid,
   ...
   FROM (reclada.v_class_lite obj
     JOIN ( SELECT 'File'::text AS class_name,
            'uri'::text AS pk
        UNION
         SELECT 'DTOJsonSchema'::text,
            'function'::text) pk ON ((pk.class_name = obj.for_class)));
...
CREATE VIEW reclada.v_ui_active_object AS
 ...
 SELECT res.obj_id,
    ...
    t.parent_guid
   FROM (res
     JOIN reclada.v_active_object t ON ((t.obj_id = res.obj_id)));


--
-- Name: v_unifields_idx_cnt; Type: VIEW; Schema: reclada; Owner: -
--

CREATE VIEW reclada.v_unifields_idx_cnt AS
 SELECT a.uni_code AS unifields_index_name,
    a.for_class,
    a.class_uuid,
    count(*) OVER (PARTITION BY a.uni_code) AS cnt
   FROM ( SELECT v_object_unifields.for_class,
            v_object_unifields.class_uuid,
            reclada.get_unifield_index_name(array_agg(v_object_unifields.unifield ORDER BY v_object_unifields.field_number)) AS uni_code,
            v_object_unifields.uni_number
           FROM reclada.v_object_unifields
          GROUP BY v_object_unifields.for_class, v_object_unifields.class_uuid, v_object_unifields.uni_number) a;
...
CREATE VIEW reclada.v_unifields_pivoted AS
 SELECT vou.class_uuid,
  ...
  GROUP BY vou.class_uuid, vou.uni_number, vou.dup_behavior, vou.is_cascade
  ORDER BY vou.class_uuid, vou.uni_number, vou.dup_behavior;


--
-- Name: cr_dup_behavior; Type: TABLE; Schema: reclada_object; Owner: -
--
...
CREATE TABLE reclada_object.cr_dup_behavior (
    ...
    last_use timestamp without time zone DEFAULT CURRENT_TIMESTAMP
);


--
-- Name: cr_dup_behavior_id_seq; Type: SEQUENCE; Schema: reclada_object; Owner: -
--

ALTER TABLE reclada_object.cr_dup_behavior ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME reclada_object.cr_dup_behavior_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
COPY reclada.draft (id, guid, user_guid, data) FROM stdin;
\.
...
88	9dc0a032-90d6-4638-956e-9cd64cd2900c	{"schema": {"type": "object", "anyOf": [{"required": ["transactionID", "class"]}, {"required": ["class"]}, {"required": ["filter", "class"]}], "properties": {"class": {"type": "string"}, "limit": {"anyOf": [{"enum": ["ALL"], "type": "string"}, {"type": "integer"}]}, "filter": {"type": "object"}, "offset": {"type": "integer"}, "orderBy": {"type": "array", "items": {"type": "object", "required": ["field"], "properties": {"field": {"type": "string"}, "order": {"enum": ["ASC", "DESC"], "type": "string"}}}}, "transactionID": {"type": "integer"}}}, "function": "reclada_object.list", "revision": "de6ff5c7-d6a7-4c9e-8537-7ffbd21b67ea"}	61	2021-12-23 09:24:28.678311+00	16d789c1-1b4e-4815-b70c-4ef060e90884	d90dd69c-fc00-4573-b747-c04f39c20b25	d87ad26e-a522-4907-a41a-a82a916fdcf9	\N
...
59	3748b1f7-b674-47ca-9ded-d011b16bbf7b	{"schema": {"type": "object", "required": ["checksum", "name", "mimeType"], "properties": {"uri": {"type": "string"}, "name": {"type": "string"}, "tags": {"type": "array", "items": {"type": "string"}}, "disable": {"type": "boolean", "default": false}, "checksum": {"type": "string"}, "mimeType": {"type": "string"}}}, "version": "1", "forClass": "File", "isCascade": true, "dupBehavior": "Replace", "dupChecking": [{"uniFields": ["uri"], "isMandatory": true}, {"uniFields": ["checksum"], "isMandatory": true}]}	58	2021-10-04 08:06:30.979167+00	16d789c1-1b4e-4815-b70c-4ef060e90884	5362d59b-82a1-4c7c-8ec3-07c256009fb0	c7fc0455-0572-40d7-987f-583cc2c9630c	\N
38	3748b1f7-b674-47ca-9ded-d011b16bbf7b	{"schema": {"type": "object", "required": ["row", "table"], "properties": {"row": {"type": "number"}, "tags": {"type": "array", "items": {"type": "string"}}, "table": {"type": "string"}, "disable": {"type": "boolean", "default": false}}}, "version": "1", "forClass": "DataRow", "parentField": "table"}	8	2021-09-22 14:52:58.301966+00	16d789c1-1b4e-4815-b70c-4ef060e90884	5362d59b-82a1-4c7c-8ec3-07c256009fb0	7643b601-43c2-4125-831a-539b9e7418ec	\N
33	3748b1f7-b674-47ca-9ded-d011b16bbf7b	{"schema": {"type": "object", "required": ["number", "bbox", "document"], "properties": {"bbox": {"type": "string"}, "tags": {"type": "array", "items": {"type": "string"}}, "number": {"type": "number"}, "disable": {"type": "boolean", "default": false}, "document": {"type": "string"}}}, "version": "1", "forClass": "Page", "parentField": "document"}	2	2021-09-22 14:52:57.011935+00	16d789c1-1b4e-4815-b70c-4ef060e90884	5362d59b-82a1-4c7c-8ec3-07c256009fb0	3ed1c180-a508-4180-9281-2f9b9a9cd477	\N
36	3748b1f7-b674-47ca-9ded-d011b16bbf7b	{"schema": {"type": "object", "required": ["bbox", "page"], "properties": {"bbox": {"type": "string"}, "page": {"type": "string"}, "tags": {"type": "array", "items": {"type": "string"}}, "disable": {"type": "boolean", "default": false}}}, "version": "1", "forClass": "Table", "parentField": "page"}	6	2021-09-22 14:52:57.379526+00	16d789c1-1b4e-4815-b70c-4ef060e90884	5362d59b-82a1-4c7c-8ec3-07c256009fb0	f5bcc7ad-1a9b-476d-985e-54cf01377530	\N
37	3748b1f7-b674-47ca-9ded-d011b16bbf7b	{"schema": {"type": "object", "required": ["colspan", "row", "text", "bbox", "table", "cellType", "rowspan", "column"], "properties": {"row": {"type": "number"}, "bbox": {"type": "string"}, "tags": {"type": "array", "items": {"type": "string"}}, "text": {"type": "string"}, "table": {"type": "string"}, "column": {"type": "number"}, "colspan": {"type": "number"}, "disable": {"type": "boolean", "default": false}, "rowspan": {"type": "number"}, "cellType": {"type": "string"}}}, "version": "1", "forClass": "Cell", "parentField": "table"}	7	2021-09-22 14:52:57.502377+00	16d789c1-1b4e-4815-b70c-4ef060e90884	5362d59b-82a1-4c7c-8ec3-07c256009fb0	7f56ece0-e780-4496-8573-1ad4d800a3b6	\N
...
85	3748b1f7-b674-47ca-9ded-d011b16bbf7b	{"schema": {"type": "object", "required": [{}, "name"], "properties": {"uri": {"type": "string"}, "name": {"type": "string"}, "tags": {"type": "array", "items": {"type": "string"}}, "disable": {"type": "boolean", "default": false}}}, "version": "1", "forClass": "DBAsset"}	84	2021-12-23 09:24:28.678311+00	16d789c1-1b4e-4815-b70c-4ef060e90884	5362d59b-82a1-4c7c-8ec3-07c256009fb0	be558164-6de8-457e-b172-e101d5712d59	f509fccd-0fb3-4d74-8d96-3c849ccbe179
84	3748b1f7-b674-47ca-9ded-d011b16bbf7b	{"schema": {"type": "object", "required": [{}, "name"], "properties": {"uri": {"type": "string"}, "name": {"type": "string"}, "tags": {"type": "array", "items": {"type": "string"}}, "disable": {"type": "boolean", "default": false}}}, "version": "1", "forClass": "Asset"}	83	2021-12-23 09:24:28.678311+00	16d789c1-1b4e-4815-b70c-4ef060e90884	5362d59b-82a1-4c7c-8ec3-07c256009fb0	f509fccd-0fb3-4d74-8d96-3c849ccbe179	92a95f66-e28e-4ebc-9f33-3568fc5a281e
63	3748b1f7-b674-47ca-9ded-d011b16bbf7b	{"schema": {"type": "object", "required": ["tasks", "command", "type"], "properties": {"tags": {"type": "array", "items": {"type": "string"}}, "type": {"type": "string"}, "tasks": {"type": "array", "items": {"type": "string", "pattern": "[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89aAbB][a-f0-9]{3}-[a-f0-9]{12}"}, "minItems": 1}, "command": {"type": "string"}, "disable": {"type": "boolean", "default": false}, "inputParameters": {"type": "string", "pattern": "[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89aAbB][a-f0-9]{3}-[a-f0-9]{12}"}, "outputParameters": {"type": "array", "items": {"type": "string", "pattern": "[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89aAbB][a-f0-9]{3}-[a-f0-9]{12}"}}}}, "version": "1", "forClass": "PipelineLite"}	62	2021-12-21 13:28:11.224553+00	16d789c1-1b4e-4815-b70c-4ef060e90884	5362d59b-82a1-4c7c-8ec3-07c256009fb0	5d3c213f-d915-456e-a10c-22d391e994d1	ba7211ce-92e5-4cbb-aa1b-a3259a9a4f54
58	3748b1f7-b674-47ca-9ded-d011b16bbf7b	{"schema": {"type": "object", "required": ["name"], "properties": {"name": {"type": "string"}, "tags": {"type": "array", "items": {"type": "string"}}, "disable": {"type": "boolean", "default": false}, "fileGUID": {"type": "string"}}}, "version": "1", "forClass": "Document", "parentField": "fileGUID"}	57	2021-10-04 08:06:30.979167+00	16d789c1-1b4e-4815-b70c-4ef060e90884	5362d59b-82a1-4c7c-8ec3-07c256009fb0	85d32073-4a00-4df7-9def-7de8d90b77e0	\N
60	3748b1f7-b674-47ca-9ded-d011b16bbf7b	{"schema": {"type": "object", "required": ["schema", "function"], "properties": {"tags": {"type": "array", "items": {"type": "string"}}, "schema": {"type": "object"}, "disable": {"type": "boolean", "default": false}, "function": {"type": "string"}}}, "version": "1", "forClass": "DTOJsonSchema"}	59	2021-11-08 11:01:49.274513+00	16d789c1-1b4e-4815-b70c-4ef060e90884	5362d59b-82a1-4c7c-8ec3-07c256009fb0	d90dd69c-fc00-4573-b747-c04f39c20b25	ab9ab26c-8902-43dd-9f1a-743b14a89825
52	3748b1f7-b674-47ca-9ded-d011b16bbf7b	{"schema": {"type": "object", "required": ["command", "status", "type", "task", "environment"], "properties": {"tags": {"type": "array", "items": {"type": "string"}}, "task": {"type": "string", "pattern": "[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89aAbB][a-f0-9]{3}-[a-f0-9]{12}"}, "type": {"type": "string"}, "runner": {"type": "string", "pattern": "[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89aAbB][a-f0-9]{3}-[a-f0-9]{12}"}, "status": {"type": "string", "enum ": ["up", "down", "idle"]}, "command": {"type": "string"}, "disable": {"type": "boolean", "default": false}, "environment": {"type": "string", "pattern": "[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89aAbB][a-f0-9]{3}-[a-f0-9]{12}"}, "inputParameters": {"type": "array", "items": {"type": "object"}}, "outputParameters": {"type": "array", "items": {"type": "object"}}, "platformRunnerID": {"type": "string"}}}, "version": "1", "forClass": "Runner"}	29	2021-09-22 14:53:04.461972+00	16d789c1-1b4e-4815-b70c-4ef060e90884	5362d59b-82a1-4c7c-8ec3-07c256009fb0	b6e879da-731f-48c3-99b5-e48d73a74930	\N
57	3748b1f7-b674-47ca-9ded-d011b16bbf7b	{"schema": {"type": "object", "required": ["tranID", "name"], "properties": {"name": {"type": "string"}, "tags": {"type": "array", "items": {"type": "string"}}, "tranID": {"type": "number"}, "disable": {"type": "boolean", "default": false}}}, "version": "1", "forClass": "ImportInfo"}	56	2021-09-24 09:46:22.790044+00	16d789c1-1b4e-4815-b70c-4ef060e90884	5362d59b-82a1-4c7c-8ec3-07c256009fb0	6f5245bd-1eec-4be0-8a28-681758155e33	\N
...
31	3748b1f7-b674-47ca-9ded-d011b16bbf7b	{"schema": {"type": "object", "required": ["Environment"], "properties": {"tags": {"type": "array", "items": {"type": "string"}}, "Lambda": {"type": "string"}, "disable": {"type": "boolean", "default": false}, "Environment": {"type": "string"}}}, "version": "1", "forClass": "Context"}	37	2021-09-22 14:52:29.56296+00	16d789c1-1b4e-4815-b70c-4ef060e90884	5362d59b-82a1-4c7c-8ec3-07c256009fb0	ea01e538-fa4d-49de-ad05-dad73a5dbaca	\N
56	3748b1f7-b674-47ca-9ded-d011b16bbf7b	{"schema": {"type": "object", "required": ["command", "name", "type", "environment"], "properties": {"name": {"type": "string"}, "tags": {"type": "array", "items": {"type": "string"}}, "type": {"type": "string"}, "command": {"type": "string"}, "disable": {"type": "boolean", "default": false}, "environment": {"type": "string", "pattern": "[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89aAbB][a-f0-9]{3}-[a-f0-9]{12}"}, "inputParameters": {"type": "string", "pattern": "[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89aAbB][a-f0-9]{3}-[a-f0-9]{12}"}, "outputParameters": {"type": "array", "items": {"type": "string", "pattern": "[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89aAbB][a-f0-9]{3}-[a-f0-9]{12}"}}, "connectionDetails": {"type": "string"}}}, "version": "1", "forClass": "Connector"}	36	2021-09-22 14:53:05.114361+00	16d789c1-1b4e-4815-b70c-4ef060e90884	5362d59b-82a1-4c7c-8ec3-07c256009fb0	6e474d14-04bb-4a82-87db-1c495664172f	\N
54	3748b1f7-b674-47ca-9ded-d011b16bbf7b	{"schema": {"type": "object", "required": ["extension", "mimeType"], "properties": {"tags": {"type": "array", "items": {"type": "string"}}, "disable": {"type": "boolean", "default": false}, "mimeType": {"type": "string"}, "extension": {"type": "string"}}}, "version": "1", "forClass": "FileExtension"}	34	2021-09-22 14:53:04.773268+00	16d789c1-1b4e-4815-b70c-4ef060e90884	5362d59b-82a1-4c7c-8ec3-07c256009fb0	e291db3a-4942-441f-a320-079c9eb5e3bb	\N
16	3748b1f7-b674-47ca-9ded-d011b16bbf7b	{"schema": {"type": "object", "required": ["channelName", "event", "class"], "properties": {"tags": {"type": "array", "items": {"type": "string"}}, "attrs": {"type": "array", "items": {"type": "string"}}, "class": {"type": "string"}, "event": {"enum": ["create", "update", "list", "delete"], "type": "string"}, "disable": {"type": "boolean", "default": false}, "channelName": {"type": "string"}}}, "version": 1, "forClass": "Message", "revision": "cc1ced42-1354-49ea-ba65-5a6562547618"}	33	2021-09-22 14:50:50.411942+00	16d789c1-1b4e-4815-b70c-4ef060e90884	5362d59b-82a1-4c7c-8ec3-07c256009fb0	54f657db-bc6a-4a37-8fb6-8566aee49b33	\N
4	3748b1f7-b674-47ca-9ded-d011b16bbf7b	{"schema": {"type": "object", "required": [], "properties": {"tags": {"type": "array", "items": {"type": "string"}}, "disable": {"type": "boolean", "default": false}}}, "version": 1, "forClass": "RecladaObject", "revision": "6035277c-e142-4a27-ae34-4800cee8c89c"}	31	2021-09-22 14:50:50.411942+00	16d789c1-1b4e-4815-b70c-4ef060e90884	5362d59b-82a1-4c7c-8ec3-07c256009fb0	ab9ab26c-8902-43dd-9f1a-743b14a89825	\N
53	3748b1f7-b674-47ca-9ded-d011b16bbf7b	{"schema": {"type": "object", "required": ["name"], "properties": {"name": {"type": "string"}, "tags": {"type": "array", "items": {"type": "string"}}, "disable": {"type": "boolean", "default": false}, "description": {"type": "string"}}}, "version": "1", "forClass": "Environment"}	30	2021-09-22 14:53:04.621778+00	16d789c1-1b4e-4815-b70c-4ef060e90884	5362d59b-82a1-4c7c-8ec3-07c256009fb0	7983c3aa-75f5-4d91-a0af-519a122893f6	\N
51	3748b1f7-b674-47ca-9ded-d011b16bbf7b	{"schema": {"type": "object", "required": ["name"], "properties": {"name": {"type": "string"}, "tags": {"type": "array", "items": {"type": "string"}}, "value": {"type": "string"}, "disable": {"type": "boolean", "default": false}}}, "version": "1", "forClass": "Value"}	28	2021-09-22 14:53:04.310403+00	16d789c1-1b4e-4815-b70c-4ef060e90884	5362d59b-82a1-4c7c-8ec3-07c256009fb0	9ac7a8c3-8d7c-4443-b5fd-c1a8f31ee13e	\N
46	3748b1f7-b674-47ca-9ded-d011b16bbf7b	{"schema": {"type": "object", "required": ["command", "status", "type", "task"], "properties": {"tags": {"type": "array", "items": {"type": "string"}}, "task": {"type": "string", "pattern": "[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89aAbB][a-f0-9]{3}-[a-f0-9]{12}"}, "type": {"type": "string"}, "runner": {"type": "string", "pattern": "[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89aAbB][a-f0-9]{3}-[a-f0-9]{12}"}, "status": {"type": "string", "enum ": ["new", "pending", "running", "failed", "success"]}, "command": {"type": "string"}, "disable": {"type": "boolean", "default": false}, "inputParameters": {"type": "array", "items": {"type": "object"}}, "outputParameters": {"type": "array", "items": {"type": "object"}}}}, "version": "1", "forClass": "Job"}	23	2021-09-22 14:53:03.806643+00	16d789c1-1b4e-4815-b70c-4ef060e90884	5362d59b-82a1-4c7c-8ec3-07c256009fb0	75a8fd8b-f709-445c-a551-e8454c0ef179	\N
45	3748b1f7-b674-47ca-9ded-d011b16bbf7b	{"schema": {"type": "object", "required": ["command", "triggers", "type"], "properties": {"tags": {"type": "array", "items": {"type": "string"}}, "type": {"type": "string"}, "command": {"type": "string"}, "disable": {"type": "boolean", "default": false}, "triggers": {"type": "array", "items": {"type": "string", "pattern": "[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89aAbB][a-f0-9]{3}-[a-f0-9]{12}"}}, "inputParameters": {"type": "string", "pattern": "[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89aAbB][a-f0-9]{3}-[a-f0-9]{12}"}, "outputParameters": {"type": "array", "items": {"type": "string", "pattern": "[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89aAbB][a-f0-9]{3}-[a-f0-9]{12}"}}}}, "version": "1", "forClass": "Pipeline"}	22	2021-09-22 14:53:03.657134+00	16d789c1-1b4e-4815-b70c-4ef060e90884	5362d59b-82a1-4c7c-8ec3-07c256009fb0	dc9eaa02-dd42-4336-bcd6-4eefe147efea	\N
44	3748b1f7-b674-47ca-9ded-d011b16bbf7b	{"schema": {"type": "object", "required": ["nextTask", "previousJobCode"], "properties": {"tags": {"type": "array", "items": {"type": "string"}}, "disable": {"type": "boolean", "default": false}, "nextTask": {"type": "string", "pattern": "[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89aAbB][a-f0-9]{3}-[a-f0-9]{12}"}, "previousJobCode": {"type": "integer"}, "paramsRelationships": {"type": "array", "items": {"type": "string", "pattern": "[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89aAbB][a-f0-9]{3}-[a-f0-9]{12}"}}}}, "version": "1", "forClass": "Trigger"}	21	2021-09-22 14:53:03.507583+00	16d789c1-1b4e-4815-b70c-4ef060e90884	5362d59b-82a1-4c7c-8ec3-07c256009fb0	a000a736-f147-4527-8849-e36efea8061e	\N
43	3748b1f7-b674-47ca-9ded-d011b16bbf7b	{"schema": {"type": "object", "required": ["name", "type"], "properties": {"file": {"type": "string"}, "name": {"type": "string"}, "tags": {"type": "array", "items": {"type": "string"}}, "type": {"enum": ["code", "stdout", "stderr", "file"], "type": "string"}, "disable": {"type": "boolean", "default": false}}}, "version": "1", "forClass": "Parameter"}	20	2021-09-22 14:53:03.359018+00	16d789c1-1b4e-4815-b70c-4ef060e90884	5362d59b-82a1-4c7c-8ec3-07c256009fb0	07533caf-3a5f-47ba-998c-494b69a9cc29	\N
39	3748b1f7-b674-47ca-9ded-d011b16bbf7b	{"schema": {"type": "object", "required": ["command", "type"], "properties": {"tags": {"type": "array", "items": {"type": "string"}}, "type": {"type": "string"}, "command": {"type": "string"}, "disable": {"type": "boolean", "default": false}, "inputParameters": {"type": "string", "pattern": "[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89aAbB][a-f0-9]{3}-[a-f0-9]{12}"}, "outputParameters": {"type": "array", "items": {"type": "string", "pattern": "[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89aAbB][a-f0-9]{3}-[a-f0-9]{12}"}}}}, "version": "1", "forClass": "Task"}	16	2021-09-22 14:53:02.985704+00	16d789c1-1b4e-4815-b70c-4ef060e90884	5362d59b-82a1-4c7c-8ec3-07c256009fb0	ba7211ce-92e5-4cbb-aa1b-a3259a9a4f54	\N
...
6	3748b1f7-b674-47ca-9ded-d011b16bbf7b	{"schema": {"type": "object", "required": ["name"], "properties": {"name": {"type": "string"}, "tags": {"type": "array", "items": {"type": "string"}}, "disable": {"type": "boolean", "default": false}}}, "version": 1, "forClass": "tag", "revision": "195b2a06-4b6e-4e7c-a610-58fc09f646c8"}	14	2021-09-22 14:50:50.411942+00	16d789c1-1b4e-4815-b70c-4ef060e90884	5362d59b-82a1-4c7c-8ec3-07c256009fb0	e12e729b-ac44-45bc-8271-9f0c6d4fa27b	\N
...
20	3748b1f7-b674-47ca-9ded-d011b16bbf7b	{"schema": {"type": "object", "required": ["caption"], "properties": {"tags": {"type": "array", "items": {"type": "string"}}, "caption": {"type": "string"}, "disable": {"type": "boolean", "default": false}}}, "version": 1, "forClass": "ObjectStatus", "revision": "0b3c19cb-85f6-4481-bd91-30d004197cac"}	11	2021-09-22 14:50:50.411942+00	16d789c1-1b4e-4815-b70c-4ef060e90884	5362d59b-82a1-4c7c-8ec3-07c256009fb0	14af3113-18b5-4da8-af57-bdf37a6693aa	\N
35	3748b1f7-b674-47ca-9ded-d011b16bbf7b	{"schema": {"type": "object", "required": ["text", "bbox", "page"], "properties": {"bbox": {"type": "string"}, "page": {"type": "string"}, "tags": {"type": "array", "items": {"type": "string"}}, "text": {"type": "string"}, "disable": {"type": "boolean", "default": false}}}, "version": "1", "forClass": "TextBlock"}	5	2021-09-22 14:52:57.258646+00	16d789c1-1b4e-4815-b70c-4ef060e90884	5362d59b-82a1-4c7c-8ec3-07c256009fb0	9ed31858-a681-49fb-9e64-250b1afaf691	\N
29	3748b1f7-b674-47ca-9ded-d011b16bbf7b	{"schema": {"type": "object", "required": ["dateTime"], "properties": {"num": {"type": "number"}, "tags": {"type": "array", "items": {"type": "string"}}, "user": {"type": "string"}, "branch": {"type": "string"}, "disable": {"type": "boolean", "default": false}, "dateTime": {"type": "string"}}}, "version": 1, "forClass": "revision"}	4	2021-09-22 14:51:54.773219+00	16d789c1-1b4e-4815-b70c-4ef060e90884	5362d59b-82a1-4c7c-8ec3-07c256009fb0	0f317fbd-8861-4d68-82ce-de7241d7db0f	\N
34	3748b1f7-b674-47ca-9ded-d011b16bbf7b	{"schema": {"type": "object", "required": ["top", "left", "height", "width"], "properties": {"top": {"type": "number"}, "left": {"type": "number"}, "tags": {"type": "array", "items": {"type": "string"}}, "width": {"type": "number"}, "height": {"type": "number"}, "disable": {"type": "boolean", "default": false}}}, "version": "1", "forClass": "BBox"}	3	2021-09-22 14:52:57.133151+00	16d789c1-1b4e-4815-b70c-4ef060e90884	5362d59b-82a1-4c7c-8ec3-07c256009fb0	c835c5b4-3b4f-49d7-b9b2-05a911234682	\N
\.


--
-- Data for Name: cr_dup_behavior; Type: TABLE DATA; Schema: reclada_object; Owner: -
--

COPY reclada_object.cr_dup_behavior (id, parent_guid, transaction_id, dup_behavior, last_use) FROM stdin;
\.
...
--
-- Name: cr_dup_behavior_id_seq; Type: SEQUENCE SET; Schema: reclada_object; Owner: -
--

SELECT pg_catalog.setval('reclada_object.cr_dup_behavior_id_seq', 1, false);
...
--
-- Name: cr_dup_behavior cr_dup_behavior_pkey; Type: CONSTRAINT; Schema: reclada_object; Owner: -
--

ALTER TABLE ONLY reclada_object.cr_dup_behavior
    ADD CONSTRAINT cr_dup_behavior_pkey PRIMARY KEY (transaction_id, parent_guid);
