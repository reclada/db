...
CREATE FUNCTION api.reclada_object_get_transaction_id(_data jsonb) RETURNS jsonb
    LANGUAGE plpgsql
    AS $$
DECLARE
    _action text;
...
SELECT pg_catalog.setval('dev.t_dbg_id_seq', 20, true);
...
SELECT pg_catalog.setval('dev.ver_id_seq', 33, true);
...
SELECT pg_catalog.setval('reclada.transaction_id', 56, true);
